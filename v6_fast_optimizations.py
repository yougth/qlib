"""
V6 引擎进阶优化压测 (M1 Mac 极速版)
=========================================================
隔离测试 3 种前沿调优手段，并打印 2019-2026 每一年的绝对收益
"""
import os, sys
...
from v5_xgb_turnover_comparative import WINDOWS, TOP80_FEATURES, build_dynamic_universe, format_qlib_code, prune_features, get_month_end_dates
...
MODEL_CONFIG = {
    ...
    "kwargs": {
        ...
        "reg_alpha": 10.0,                
        "reg_lambda": 50.0,               
        "tree_method": "hist",  # M1 提速核心：直方图加速
        "n_jobs": 4,            # 限制4核：防内存爆满与过热降频
...
def run_experiments():
    ...
    print(f"{'='*110}")
    print(f"{'策略分支':<20} | {'2019':>7} {'2021':>7} {'2022':>7} {'2023':>7} {'2024':>7} {'2025':>7} {'2026H1':>7} | {'全期净年化':>10} {'夏普':>6} {'最大回撤':>9}")
    print(f"{'-'*110}")
    for _, r in df_res.iterrows():
        print(f"{r['实验分支']:<18} | {r['2019']*100:>6.2f}% {r['2021']*100:>6.2f}% {r['2022']*100:>6.2f}% {r['2023']*100:>6.2f}% {r['2024']*100:>6.2f}% {r['2025']*100:>6.2f}% {r['2026H1']*100:>6.2f}% | {r['全期净年化']*100:>9.2f}% {r['净夏普']:>6.2f} {r['净最大回撤']*100:>8.1f}%")
    print(f"{'='*110}\n")
